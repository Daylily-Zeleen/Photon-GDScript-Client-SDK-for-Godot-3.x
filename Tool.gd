...
const LOADBALANCING_API_PATH := "res://addons/Photon/dependents/PhotonLoadbalancingApi/"
const CHAT_API_PATH :="res://addons/Photon/dependents/PhotonChatApi/"
...
用于调试：
  <ItemGroup>
	<Reference  Include="addons\\Photon\\dependents\\libs\\Debug\\netstandard2.0\\Photon-NetStandard.dll" />
</ItemGroup> 

用于发布：
  <ItemGroup>
	<Reference  Include="addons\\Photon\\dependents\\libs\\Release\\netstandard2.0\\Photon-NetStandard.dll" />
</ItemGroup> 
==""")
		...
		if dir.list_dir_begin() ==OK:
			...
			if included :
				if dir.dir_exists(LOADBALANCING_API_PATH) and dir.dir_exists(CHAT_API_PATH):
					...
					printerr("目录 'res://addons/Photon/dependents/' 下未包含 PhotonLoadbalancingApi 和 PhotonChatApi")
	return false
